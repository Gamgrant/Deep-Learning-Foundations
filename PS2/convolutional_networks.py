...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_padding = torch.nn.functional.pad(x, (pad, pad, pad, pad))
        out = torch.zeros((N, F, H_out, W_out), dtype=x.dtype, device = x.device)      
        for i in range(F):
          for j in range(H_out):
            for k in range(W_out):
              x_slice = x_padding[:,:,j*stride:j*stride+HH, k*stride:k*stride+WW]
              out[:, i, j, k] = torch.sum(x_slice * w[i], dim=(1,2,3)) + b[i]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros(x_padding.shape, dtype=x.dtype, device=x.device)
        
        for i in range(N):
          for j in range(F):
            for height in range(H_out):
              for weight in range(W_out):
                dx_slice = x_padding[i, : , height*stride:height*stride+HH, weight*stride:weight*stride+WW]
                dw[j, :, :, :] += dout[i, j, height, weight] * dx_slice 
                dx[i, :, height*stride:height*stride+HH,weight*stride:weight*stride+WW] += dout[i, j, height, weight] * w[j, :, :, :] 
    ...
    def forward(x, pool_param):
        ...
        for i in range(N):
          for j in range(C):
            for height in range(H_out):
              for weight in range(W_out):
                x_slice = x[i, j, height*stride : height*stride+pool_height, weight*stride : weight*stride+pool_width]
                out[i, j, height, weight] = torch.max(x_slice)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
        ...
        for i in range(N):
          for j in range(C):
            for height in range(H_out):
              for weight in range(W_out):
                x_slice = x[i, j, height*stride:height*stride+pool_height, weight*stride:weight*stride+pool_width]
                maxValue = torch.max(x_slice)
                maxIdx = (x_slice == maxValue)
                # print(maxIdx)
                # only the position of the maximum element in the region i,j gets the incoming gradient, the other gradients are zero
                dx[i, j, height*stride:height*stride+pool_height,weight*stride:weight*stride+pool_width][maxIdx] = dout[i, j, height, weight]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += torch.sum(torch.pow(self.params['W1'], 2)) * self.reg
        loss += torch.sum(torch.pow(self.params['W2'], 2)) * self.reg
        loss += torch.sum(torch.pow(self.params['W3'], 2)) * self.reg

        dh2, dw3, db3 = Linear.backward(dout, cache3)
        dh1, dw2, db2 = Linear_ReLU.backward(dh2, cache2)
        _, dw1, db1 = Conv_ReLU_Pool.backward(dh1, cache1)

        grads['W1'] = dw1 + self.params['W1'] * 2 * self.reg
        grads['W2'] = dw2 + self.params['W2'] * 2 * self.reg
        grads['W3'] = dw3 + self.params['W3'] * 2 * self.reg
        grads['b1'] = db1
        grads['b2'] = db2
        grads['b3'] = db3
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
          self.params['W1'] = kaiming_initializer(C, num_filters[0], 3, relu=True, device=device, dtype=dtype)
          ...
          self.params['b1'] = torch.zeros(num_filters[0], device=device, dtype=dtype)
        # normalize the output of each layer by subtracting the mean and dividing by the variance of the batch of inputs.
        if self.batchnorm:
        ...
        for i in range(2, self.num_layers):
          if weight_scale == 'kaiming':
            self.params[f'W{i}'] = kaiming_initializer(num_filters[i-2], num_filters[i-1], 3, relu=True, device=device, dtype=dtype)
            self.params[f'b{i}'] = torch.zeros(num_filters[i-1], device=device, dtype=dtype)
          else:
            self.params[f'W{i}'] = torch.randn(num_filters[i-1], num_filters[i-2], 3, 3, device=device, dtype=dtype) * weight_scale
            self.params[f'b{i}'] = torch.zeros(num_filters[i-1], device=device, dtype=dtype)
        ...
        final_dim = num_filters[-1] * (H // (2 ** len(max_pools)))* (W // (2 ** len(max_pools)))
        if weight_scale == 'kaiming':
          self.params[f'W{self.num_layers}'] = kaiming_initializer(final_dim, num_classes, relu=False, dtype=dtype, device=device)
          self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=dtype, device=device)
        else:
          self.params[f'W{self.num_layers}'] =  torch.randn(final_dim, num_classes, device=device, dtype=dtype) * weight_scale
          self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.batchnorm:
                ...
                if i in self.max_pools:
                    input_data, caches[i] = Conv_BatchNorm_ReLU_Pool.forward(input_data, weights, biases, gamma, beta, conv_param, self.bn_params[i], pool_param)
                else:
                    input_data, caches[i] = Conv_BatchNorm_ReLU.forward(input_data, weights, biases, gamma, beta, conv_param, self.bn_params[i])
            else:
                if i in self.max_pools:
                    input_data, caches[i] = Conv_ReLU_Pool.forward(input_data, weights, biases, conv_param, pool_param)
                else:
                    input_data, caches[i] = Conv_ReLU.forward(input_data, weights, biases, conv_param)
        output_weights = self.params[f'W{self.num_layers}']
        output_biases = self.params[f'b{self.num_layers}']

        scores, caches[self.num_layers - 1] = Linear.forward(input_data, output_weights, output_biases)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        # Compute gradients of the last layer (linear layer)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = torch.sum(torch.pow(x-sample_mean, 2), dim=0)/N
            out = (x - sample_mean) / torch.sqrt(sample_var + eps)
            out = out * gamma + beta
            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var

            cache = (gamma, x, x-sample_mean, sample_var, 1/torch.sqrt(sample_var + eps), eps, (x - sample_mean)* (1/torch.sqrt(sample_var + eps)), mode)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            cache = (gamma, x, running_mean, running_var, x_norm, mode)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        if mode == 'train': 
          ...
          d_xi = dout * gamma          
          dvar =  torch.sum(d_xi * mean * (-0.5) * torch.pow(var+eps, -3/2), dim=0) 
          dmu = -1*torch.sum(d_xi * ivar, dim = 0) + dvar * torch.sum(-2* mean * (torch.ones_like(x) / N * dvar), dim = 0)
          dx = ((d_xi * ivar) + (2 * mean * (torch.ones_like(x) / N * dvar))) + (dmu * torch.ones_like(x) / N)
        else:
          gamma, x, mean, var, running_var, eps, x_norm, _ = cache
          dbeta = torch.sum(dout, dim = 0)
          dgamma = torch.sum(dout * x_norm, dim = 0)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        out, cache = BatchNorm.forward(x.reshape(N * H * W, C), gamma, beta, bn_param)
