...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        new_x = x.view(N ,-1)  #(N, D)
        out = torch.mm(new_x, w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        relu, cache1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache2 = Linear.forward(relu, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss += self.reg * (torch.sum(torch.pow(self.params['W1'], 2)) + torch.sum(torch.pow(self.params['W2'], 2)) )
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          W = self.params[f'W{i}']
          b = self.params[f'b{i}']
          h, caches[i] = Linear_ReLU.forward(h, W, b)
        ...
        for i in range(1, self.num_layers + 1):
          loss += 0.5 * self.reg * torch.sum(torch.pow(self.params[f'W{i}'], 2))
        ...
        for i in range(self.num_layers - 1, 0, -1):
          dh, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dh, caches[i])
