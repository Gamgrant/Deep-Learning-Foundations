...
def softmax_loss_naive(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    numOfClass =  W.shape[1]
    numOfTrain = X.shape[0]
    for i in range(numOfTrain):
      scores = torch.mv(W.T, X[i]) #dim of output is C
...
def softmax_loss_vectorized(
    ...
    # test = torch.max(scores, dim=1).values
    # print(test.shape)
    # must to keep the dimension, or the shape after max will be ([128]) instead of ([128, 1])
    scores -= torch.max(scores, dim = 1, keepdim=True).values
    p = torch.exp(scores)/torch.sum(torch.exp(scores), dim = 1, keepdim=True)
    # print(p.shape)
    # print(torch.arange(numOfTrain).shape)
    all_correct = p[torch.arange(numOfTrain), y]
    loss -= torch.sum(torch.log(all_correct))
    loss /= numOfTrain
    loss += reg*torch.sum(W*W)

    p[torch.arange(numOfTrain), y] -= 1
    dW = torch.mm(X.T, p)
    dW /= numOfTrain
    dW += 2*reg*W
    #############################################################################
