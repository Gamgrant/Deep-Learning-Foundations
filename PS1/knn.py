...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i][j] = torch.sum(torch.pow(x_train_flatten[i] - x_test_flatten[j], 2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten = x_test.view(num_test, -1)
    for i in range(num_train):
      dists[i] = torch.sum(torch.pow(x_train_flatten[i] - x_test_flatten, 2), dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flatten = x_train.view(num_train, -1)
    x_test_flatten = x_test.view(num_test, -1)
    # temp = torch.sum(torch.pow(x_train_flatten, 2), dim = 1)
    # print(temp.shape)
    x_train_sq = torch.sum(torch.pow(x_train_flatten, 2), dim = 1, keepdim = True)
    # print(x_train_sq.shape)
    x_test_sq = torch.sum(torch.pow(x_test_flatten, 2), dim = 1)
    dists = x_train_sq + x_test_sq - 2 * torch.mm(x_train_flatten, x_test_flatten.T)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    y_pred = torch.mode(y_train[topk_indices], dim = 0).values
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in k_choices:
      ...
      for j in range(num_folds):
        x_tain_new = torch.cat(x_train_folds[0 : j] + x_train_folds[j + 1 :])
        y_tain_new = torch.cat(y_train_folds[0 : j] + y_train_folds[j + 1 : ])
        knn = KnnClassifier(x_tain_new, y_tain_new)
        k_to_accuracies[i].append(knn.check_accuracy(x_train_folds[j], y_train_folds[j], i))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for i in k_to_accuracies:
      # res.append(torch.mean(torch.tensor(k_to_accuracies[i])).item())
      cur = torch.mean(torch.tensor(k_to_accuracies[i])).item()
      if cur > pre:
        best_k = i
        pre = cur
    ##########################################################################
