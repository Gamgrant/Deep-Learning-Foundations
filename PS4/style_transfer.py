...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      ...
      A = style_targets[i]
      loss += style_weights[i] * torch.sum(torch.pow((G - A), 2))
...
def tv_loss(img, tv_weight):
    ...
    # Replace "Pass" statement with your code
    t_var = 0
    temp1 = torch.sum(torch.pow((img[:, :, 1:, :] - img[:, :, :-1, :]), 2))
    temp2 = torch.sum(torch.pow((img[:, :, :, 1:] - img[:, :, :, :-1]), 2))
    t_var = tv_weight * (temp1 + temp2)
    return t_var      
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  features_flat = features.view(N, R, C, H*W)
  mask = masks.view(N, R, 1, H*W)

  for r in range(R):
    f = features_flat[:, r, :, :] * mask[:, r, :, :]
    f_t = torch.transpose(f, 1, 2) 
    guided_gram[:, r, :, :] = f @ f_t
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    Inputs:
    - feats: list of the features at every layer of the current image, as produced by
      the extract_features function.s
    - style_layers: List of layer indices into feats giving the layers to include in the
      style loss.
    - style_targets: List of the same length as style_layers, where style_targets[i] is
    ...
    loss = 0
    for i in range(len(style_layers)):
      G = guided_gram_matrix(feats[style_layers[i]], content_masks[style_layers[i]])
      A = style_targets[i]
      loss += style_weights[i] * (torch.sum(torch.pow((G - A), 2)))
    return loss
    ############################################################################
